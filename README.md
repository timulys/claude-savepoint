...
/load  → 최근 스냅샷 10개를 목록으로 보여주고, 골라서 요약·작업 재개
/log   → 지난 세션 목록 보기
```
...
| `LATEST.md` | 가장 최근 스냅샷 사본 — `/load latest` 단축 경로용 |
| `INDEX.md` | 모든 스냅샷의 한 줄 인덱스 — `/log`가 읽음 |

### `/load`

`INDEX.md`에서 **최근 세션 10개를 번호 목록으로 보여주고**, 사용자가 고른 세션을 요약해 작업을 재개합니다. (여러 세션을 병렬로 돌리면 `LATEST.md`는 마지막 저장본으로 덮어써지므로, 누적 인덱스에서 골라야 직전 세션이 묻히지 않습니다.) 단순히 본문을 그대로 출력하지 않고 **"어디서부터 이어갈지"를 추출**해 보여줍니다:
...
목록을 건너뛰는 단축 경로도 있습니다:

- `/load latest` — 최신 세션 바로 로드
- `/load 2026-05-04` — 해당 날짜 스냅샷
- `/load HISTORY-2026-05-04-1530` — 정확한 파일명
- `/load list` — 전체 이력 보기 (`/log`로 위임)
